...
router.get('/', (req, res) => {
  ...
  const done    = db.prepare('SELECT COUNT(*) AS n FROM todos WHERE userId = ? AND done = 1').get(uid).n;
  const overdue = db.prepare(
    'SELECT COUNT(*) AS n FROM todos WHERE userId = ? AND done = 0 AND dueDate IS NOT NULL AND dueDate < ?',
  ).get(uid, now).n;

  const byPriority = { low: 0, medium: 0, high: 0 };
  for (const row of db.prepare('SELECT priority, COUNT(*) AS n FROM todos WHERE userId = ? GROUP BY priority').all(uid)) {
