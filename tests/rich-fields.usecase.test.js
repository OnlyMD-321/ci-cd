...
describe('rich-fields — use cases', () => {
  describe('POST /todos with priority and dueDate', () => {
    it('creates a todo with default priority medium when omitted', async () => {
      const res = await request(app).post('/todos').set('Authorization', `Bearer ${token}`).send({ title: 'task' });
    ...
    it('creates a todo with priority high', async () => {
      const res = await request(app).post('/todos').set('Authorization', `Bearer ${token}`).send({ title: 'urgent', priority: 'high' });
      expect(res.status).toBe(201);
      expect(res.body.priority).toBe('high');
    });

    it('creates a todo with priority low', async () => {
      const res = await request(app).post('/todos').set('Authorization', `Bearer ${token}`).send({ title: 'someday', priority: 'low' });
      expect(res.status).toBe(201);
      expect(res.body.priority).toBe('low');
    });

    it('creates a todo with a dueDate', async () => {
      const res = await request(app).post('/todos').set('Authorization', `Bearer ${token}`).send({ title: 'deadline', dueDate: '2025-12-31' });
      expect(res.status).toBe(201);
      expect(res.body.dueDate).toBe('2025-12-31');
    });

    it('creates a todo with both priority and dueDate', async () => {
      const res = await request(app).post('/todos').set('Authorization', `Bearer ${token}`).send({ title: 'critical', priority: 'high', dueDate: '2025-06-01' });
      expect(res.status).toBe(201);
  ...
  describe('PUT /todos/:id updates rich fields', () => {
    it('updates priority on an existing todo', async () => {
      const created = await request(app).post('/todos').set('Authorization', `Bearer ${token}`).send({ title: 'task' });
      const res = await request(app).put(`/todos/${created.body.id}`).set('Authorization', `Bearer ${token}`).send({ title: 'task', done: false, priority: 'high' });
    ...
    it('updates dueDate on an existing todo', async () => {
      const created = await request(app).post('/todos').set('Authorization', `Bearer ${token}`).send({ title: 'task' });
      const res = await request(app).put(`/todos/${created.body.id}`).set('Authorization', `Bearer ${token}`).send({ title: 'task', done: false, dueDate: '2026-01-01' });
      expect(res.status).toBe(200);
      expect(res.body.dueDate).toBe('2026-01-01');
    });

    it('clears dueDate by setting it to null', async () => {
      const created = await request(app).post('/todos').set('Authorization', `Bearer ${token}`).send({ title: 'task', dueDate: '2025-12-31' });
      const res = await request(app).put(`/todos/${created.body.id}`).set('Authorization', `Bearer ${token}`).send({ title: 'task', done: false, dueDate: null });
      expect(res.status).toBe(200);
      expect(res.body.dueDate).toBeNull();
    });
  });
